...
my $client = Gearman::Client::Async->new(exceptions => 1);
isa_ok($client, 'Gearman::Client::Async');
$client->set_job_servers(map { '127.0.0.1:' . (PORT + $_) } 0..$NUM_SERVERS);
...
isa_ok($tasks, 'Gearman::Client::Async');
my $sum;
my $failed = 0;
my $completed = 0;
